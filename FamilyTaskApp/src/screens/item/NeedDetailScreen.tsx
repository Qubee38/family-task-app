...
type NeedDetailScreenNavigationProp = NativeStackNavigationProp<RootStackParamList>;
...
export default function NeedDetailScreen() {
    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>必要物詳細</Text>
          <View style={styles.headerRight} />
        </View>
        <View style={styles.errorContainer}>
          <Text style={styles.errorText}>必要物IDが指定されていません</Text>
        </View>
      </View>
    );
  }
  ...
  if (!item) {
    return (
      <View style={styles.container}>
        <View style={styles.errorContainer}>
          <Text style={styles.errorText}>必要物が見つかりません</Text>
        </View>
      </View>
    );
  }
  ...
  const handleEdit = () => {
    navigation.navigate('CreateEditNeed', { itemId: item.itemId });
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      logger.error('必要物削除エラー:', error);
      const message = error.response?.data?.detail || error.message || '必要物の削除に失敗しました';
      if (Platform.OS === 'web') {
        alert(message);
      } else {
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content}>
        ...
        <View style={styles.detailsCard}>
          ...
          {item.location && (
            <View style={styles.detailRow}>
              <Text style={styles.detailLabel}>購入場所</Text>
          ...
          {item.isCompleted && item.completedByName && (
            <>
              <View style={styles.detailRow}>
                <Text style={styles.detailLabel}>購入者</Text>
                <Text style={styles.detailValue}>{item.completedByName}</Text>
              </View>

              {item.completedAt && (
                <View style={styles.detailRow}>
                  <Text style={styles.detailLabel}>購入日時</Text>
        ...
        >
          {loading ? (
            ...
            <Text style={styles.actionButtonText}>
              {item.isCompleted ? '未購入に戻す' : '購入済みにする'}
            </Text>
          )}
        </TouchableOpacity>
      </ScrollView>
